...
<html>
  ...
  <script src="resources/js/templates.js" type="text/javascript"></script>
  ...
  $(document).ready(function() {
    ...
    var runningInvoice = {
      ...
      addProduct: function(product) {
        ...
        line.lineNumber = this.lines.length + 1;
        line.unitCost = product.unitCost;
        this.lines.push(line);
        return line;
      },
      updateQuantity: function(index, qty) {
        this.lines[index].quantity = parseInt(qty);
        this._updateTotal();
      },
      ...
      _updateTotal: function() {
        this.subTotal = this.lines.reduce( ((p,c) => p+c.unitCost*c.quantity), 0);
        this.tax = this.subTotal * globalTax;
    ...
    var updateTable = function() {
      $("#edit-invoice-table tbody tr").each(function(i) {
        var line = runningInvoice.lines[i];
        $(this).find(".invoice-line-number span").text(line.lineNumber);
        $(this).find(".invoice-total-item-cost").text(currencyFormat(line.unitCost * line.quantity));
      });
      $("#invoice-subtotal").text(currencyFormat(runningInvoice.subTotal));
      $("#invoice-tax").text(currencyFormat(runningInvoice.tax));
    ...
    var setupForm = function() {
      ...
      $("#edit-invoice-table").on("click", ".product-select", function() {
        ...
        var selectedProducts = runningInvoice.lines.map(line => line.product);
        products
          .filter(product => selectedProducts.indexOf(product) === -1 && product.stock.numInStock > 0)
        ...
        deferred.done(function(product) {
          ...
          $("<span></span>").appendTo(col1).text(line.lineNumber);
          var col2 = $("<td></td>").text(getFormattedProductName(line.product)).addClass("invoice-product-name");
          var col3 = $("<td></td>").text(currencyFormat(line.unitCost)).addClass("invoice-unit-cost");
          var col4 = $("<td></td>");
          $(`<input type="number" style="width:60px" class="invoice-line-qty-select" value="0" />`).appendTo(col4);
          var col5 = $("<td></td>").text(currencyFormat(line.unitCost * line.quantity)).addClass("invoice-total-item-cost");
      ...
      $("#edit-invoice-table").on("change", ".invoice-line-qty-select", function() {
        ...
        var currStock = runningInvoice.lines[lineNum].product.stock.numInStock;
